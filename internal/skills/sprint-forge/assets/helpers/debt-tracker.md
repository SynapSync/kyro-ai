...
If the runtime or verb is unavailable, STOP without mutation. Report the observed version (or `not installed`) and `npm install -g kyro-ai`, then from the project root `kyro sync --scope workspace --yes`; never edit `debt[]` by hand.
