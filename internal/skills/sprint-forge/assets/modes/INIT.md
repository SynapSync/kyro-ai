...
`npm install -g kyro-ai`, then from the project root global `kyro` with `install --scope workspace --init-workspace --yes` once (full npm package only, never
