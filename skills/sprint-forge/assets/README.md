...
| [kyro.json](templates/kyro.json) | **Legacy** monolito registry. Superseded by layered `project.json` + `local.json`, which `{{KYRO_CLI}} plan` writes. Kept for reading pre-migration projects only — never author a new one. |
