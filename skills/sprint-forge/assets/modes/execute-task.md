...
## Opt-in delegated execution (L0)

**Default:** execute as a single agent (steps 1–3 above). **Opt-in:** when the user asks to delegate the current task to a worker/subagent, or when `context-pack` reports `delegationEnabled: true`, delegate implementation to a worker and keep workflow ownership.

### L1 routing (`delegationEnabled` from context-pack)

When the task pack JSON includes `"delegationEnabled": true`:

1. Load `../delegates/implementer.md` — role contract for the worker.
...
When `delegationEnabled` is `false` or absent, use single-agent steps 1–3 only (L0 prose applies when the user explicitly asks for delegation anyway).

### When to use

- User explicitly requests delegated execution (e.g. "run this task with a delegate").
- Orchestrator chooses delegation for a large, well-bounded task — still opt-in, never the default for every task.

### Delegate brief (orchestrator builds; worker receives)
...
### Delegate status contract (worker → orchestrator)
...
| **Delegate** | Edit product code, run local validation, return status JSON | Mutate workflow state files; self-approve review |
| **Kyro CLI** | Canonical workflow writes (`record-evidence`, `review`, …) | — |

### Phase UX with delegation

"Run the phase with delegates" means: **orchestrator loops** ready tasks in the phase (`for each task → brief → delegate → apply status → CLI`). It does **not** mean one delegate owns the phase or sprint.

### Fallback when subagents are unavailable

If the host cannot spawn a worker (no Task/subagent tool, spawn fails, or user did not opt in): **fall back silently** to the single-agent workflow (steps 1–3). Never block the forge because delegation is unavailable.
...
See also: `docs/architecture.md` (delegated execution protocol), `docs/context-management.md` (task packs), `docs/maker-checker.md` (checker delegate).
