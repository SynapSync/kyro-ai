...
| Moment | Write only |
|--------|------------|
| Task close | Append one JSON line to `events.ndjson`: timestamp, scope, sprint, phase, task, status, changedFiles, validation, blockers, debtDeltas, notes. |
...
4. Record concrete evidence as compact `events.ndjson` task events, not full rewritten retros.
...
- Do not defer all progress tracking to sprint close; keep `events.ndjson` current.
