...
Preferred: `{{KYRO_CLI}} plan --from <file> --kyro-scope {scope}` (tool-owned, validated). Fallback: hand-write using the Artifact Write Contract in `../../SKILL.md` (read → parse → mutate object → overwrite whole file → re-parse):
