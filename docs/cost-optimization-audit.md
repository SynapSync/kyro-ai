...
| Task execution encouraged repeated artifact rewrites | High output/input churn | Use `events.ndjson` during execution and materialize at close |
| Rules loaded as full Markdown | Growing rules ledger would increase startup cost | Add `rules.index.json` |
