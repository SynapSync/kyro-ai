...
export function buildUpdatePlan(facts: UpdateFacts): UpdatePlan {
  const { current, latest, runtimeVersion, hasWorkspace, hasLegacyScopeCache, durableGlobal } = facts;
  ...
  if (!behind && runtimeVersion && compareSemverLike(runtimeVersion, current) === -1) {
    const refresh = hasWorkspace ? 'sync the current workspace' : 'refresh the global runtime';
    return {
      facts,
      action: 'refresh-stale-runtime',
      target: current,
      behind,
      steps: [`${refresh} from this package (no download needed)`],
      summary: `CLI is current (${current}) but the installed runtime is older (${runtimeVersion})`,
    };
  }

  if (!behind && hasWorkspace && hasLegacyScopeCache) {
    const validRuntimeVersion = validateTargetVersion(runtimeVersion);
    const target = validRuntimeVersion && compareSemverLike(validRuntimeVersion, current) === 1
      ? validRuntimeVersion
      : current;
    return {
      facts,
      ...
      target,
      behind,
      steps: [`sync the current workspace from kyro ${target} to remove legacy project.json.scopes[]`],
      summary: `workspace has a legacy scopes[] cache; refresh it from kyro ${target}`,
    };
  }
...
export async function runUpdate(options: CliOptions): Promise<void> {
  ...
  const facts: UpdateFacts = {
    ...
    hasLegacyScopeCache: Object.prototype.hasOwnProperty.call(readSharedProjectState() ?? {}, 'scopes'),
  ...
  if (plan.action === 'refresh-stale-runtime' || plan.action === 'refresh-workspace') {
    ...
    if (detectPackageRootMode() === 'full-package' && plan.target === current) {
      refreshFromCli(currentCliJs(), facts.hasWorkspace, options.verbose);
    } else {
      const refreshArgs = facts.hasWorkspace
        ? ['sync', '--scope', 'workspace']
        : ['install', '--scope', 'workspace', '--no-init-workspace'];
      const args = ['-y', `${UPDATE_PACKAGE}@${plan.target}`, ...refreshArgs];
      if (options.verbose) args.push('--verbose');
      const refreshed = runPackageManager('npx', args, { stdio: 'inherit' });
      if (refreshed.spawnError || refreshed.status !== 0) {
        throwInstallFailed(refreshed.spawnError ?? `exit code ${String(refreshed.status)}`, refreshed.stderr);
      }
    }
    console.log(facts.hasWorkspace
      ? `Workspace and runtime refreshed from kyro ${plan.target}.`
      : `Runtime refreshed from kyro ${plan.target}.`);
    return;
  }
