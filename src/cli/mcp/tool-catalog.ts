...
export const MCP_TOOLS = [
  ...
  },
  {
    ...
    inputSchema: { type: 'object', properties: { scope, task_id: taskId, verdict: { type: 'string', enum: ['pass', 'fail'], description: 'Reviewer judgment. Defaults to pass.' }, checked_criteria: { type: 'array', items: { type: 'string' }, description: 'Acceptance criteria the reviewer checked. Defaults to all on a pass.' }, findings: { type: 'array', items: { type: 'string' }, description: 'Reviewer findings as "severity:detail" (severity = critical|warning|suggestion).' }, by: { type: 'string', description: 'Reviewer actor id. Defaults to "checker".' }, confirm }, required: ['task_id'], additionalProperties: false },
    annotations: { destructiveHint: true, idempotentHint: false },
  },
  {
    name: 'trace_tail',
